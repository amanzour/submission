...
from src.data.sec_struct_utils import get_unpaird_amir
...
class RNAGraphFeaturizer(object):
    ...
    def __call__(self, rna):
        with torch.no_grad():
            ...
            #=========amir=========================================
            # Set of coordinates: num_conf x num_res x num_bb_atoms x 3
            coords_list, basepairs_amir, mask_coords, mask_confs = get_k_random_entries_and_masks_amir(
                rna['coords_list'], rna['sec_bp_list'], k = self.max_num_conformers
            )
            #==================end amir============================
            ...
            if (mask_coords == False).sum() > 0:
                basepairs_amir = offset_basepairs_amir(basepairs_amir,mask_coords)
            ...
            # ))
            # ===============amir============================
            unpaired_amir = [get_unpaird_amir(len(seq),basepairs_amir[i]) for i in range(len(basepairs_amir))]
            all_idx_amir =  torch.tensor([i for i in range(len(seq))])
            # primary structure edges
            edge_index_primary = torch.stack((all_idx_amir[:-1],all_idx_amir[1:]), dim = 0)
            edge_index_primary_amir = to_undirected(edge_index_primary)
            edge_index_secondary_amir = []
            edge_index_tertiary_amir = []
            for k in range(self.max_num_conformers):
                ...
                edge_index_secondary_amir.append(edge_index_secondary)
                
                # tertiary structure edges
                edge_index_tertiary = to_undirected(unpaired_cluster_dbscan_amir(unpaired_amir[k], coords_list[k], primary_dist = 500, dbscan_eps = 10))                
                edge_index_tertiary_amir.append(edge_index_tertiary)
            ...
            for k in range(self.max_num_conformers):
                primary = coords_list[k][edge_index_primary_amir[0]] - coords_list[k][edge_index_primary_amir[1]]
                secondary=torch.empty(0,dim1,dim2)
                if edge_index_secondary_amir[k].shape[1] > 0:
                    secondary = coords_list[k][edge_index_secondary_amir[k][0]] - coords_list[k][edge_index_secondary_amir[k][1]]
                tertiary=torch.empty(0,dim1,dim2)
                if edge_index_tertiary_amir[k].shape[1] > 0:
                    tertiary = coords_list[k][edge_index_tertiary_amir[k][0]] - coords_list[k][edge_index_tertiary_amir[k][1]]
                cur_edge_index = torch.cat((edge_index_primary_amir,edge_index_secondary_amir[k],edge_index_tertiary_amir[k]), dim = -1)
                ...
                cur_p_s_t_map = torch.cat((p_mask.repeat(edge_index_primary_amir.shape[-1]),
                                           s_mask.repeat(edge_index_secondary_amir[k].shape[-1]),
                                           t_mask.repeat(edge_index_tertiary_amir[k].shape[-1])), dim = 0)
            ...
            for k in range(self.max_num_conformers):
                ...
                cur_edge_posenc = positional_encoding((edge_index_list[k][0] - edge_index_list[k][1])[..., None], 
                                                      self.num_posenc)
                edge_s_list.append(torch.cat([cur_edge_rbf, cur_edge_posenc, torch.log(edge_lengths_list[k])], dim=-1))
        ...
        data_amir = torch_geometric.data.Data(
            seq = seq,                  # num_res x 1
            node_s = node_s,            # num_res x num_conf x (num_bb_atoms x 5)
            node_v = node_v,            # num_res x num_conf x (2 + (num_bb_atoms - 1)) x 3
            edge_s_list = edge_s_list,            # num_conf x num_edges x (num_bb_atoms x num_rbf + num_posenc + num_bb_atoms)
...
def spheric2cartesian_amir(r, theta, phi):
    x = r*torch.cos(theta) *torch.sin(phi)
    y = r*torch.sin(theta)*torch.sin(phi)
    z= r*torch.cos(phi)
    return x, y, z

def get_k_random_entries_and_masks_amir(coords_list, basepairs, k):
    """
    Returns k random entries from a list of 3D coordinates, along with
    the corresponding masks (1 = valid, 0 = not valid).
...
def unpaired_cluster_dbscan_amir(unpaired_idx, coord, primary_dist = 500, dbscan_eps = 10):
    edges_source = []
    edges_dest = []
    coord_mean = coord.mean(1)
    unpaired_coords_mean = coord_mean[unpaired_idx]
    if unpaired_coords_mean.shape[0] > 0:
        db = DBSCAN(eps=10, min_samples=5).fit(unpaired_coords_mean)
    else:
        dummy = 2
        ...
        if cur_label_idx != -1:
            ...
            for i in range(len(idx_mem)-1):
                for j in range(i+1, len(idx_mem)):
                    if (abs(idx_mem[i] - idx_mem[j]) > primary_dist) & (2 < dist_amir(coord_mean[idx_mem[i]],coord_mean[idx_mem[j]]).item() < 2*dbscan_eps):
...
def offset_basepairs_amir(basepairs,masks):
    return_bp = []
