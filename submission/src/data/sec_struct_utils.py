...
def pdb_to_sec_struct_amir(
    ...
    if 1 < len(sequence) < 4000:
        try:
            sec_struct = x3dna_to_sec_struct_amir(
            pdb_to_x3dna_amir(pdb_file_path, x3dna_path), 
            sequence
        )
        ...
        try:
            fr3d_sec_struct = fr3d_to_sec_struct_amir(fr3d_file_path, sequence, pdb_map)
...
def x3dna_to_sec_struct_amir(output: List[str], sequence: str) -> list:
    ...
    for i in range(1, num_base_pairs+1):
        line = output[4 + i].split()
        start, end = int(line[0]), int(line[1])
        list_amir.append([start,end])  
    return list_amir

def fr3d_to_sec_struct_amir(fr3d_file_path, sequence, pdb_map):
    list_amir = []
    try:
